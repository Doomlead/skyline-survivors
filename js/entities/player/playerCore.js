...
function playerDie(scene) {
    ...
    if (isVeritechActive) {
        ...
        if (gameState.rebuildObjective) {
            ...
            gameState.rebuildObjective.branch = hasHangar ? 'hangar' : (gameState.rebuildObjective.branch || 'dropship');
            gameState.rebuildObjective.stage = hasHangar ? 'secure_hangar' : 'secure_extraction';
            ...
            if (hasHangar && typeof showRebuildObjectiveBanner === 'function') {
                showRebuildObjectiveBanner(scene, 'Reach the hangar and hold position (30s)');
            }
        }
