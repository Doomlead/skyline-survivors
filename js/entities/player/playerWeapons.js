    ...
    switch (weapon) {
        case 'scattergun': {
            ...
            for (let i = 0; i < pellets; i++) {
                ...
                createProjectile(scene, origin.fireX, origin.fireY, Math.cos(angle) * 680, Math.sin(angle) * 680, 'normal', damage);
            }
        ...
        case 'plasmaLauncher': {
            ...
            createProjectile(scene, origin.fireX, origin.fireY, Math.cos(baseAngle) * 480, Math.sin(baseAngle) * 480, 'wave', damage, { piercing: tier >= 3 });
            state.ammo.plasmaLauncher = Math.max(0, (state.ammo.plasmaLauncher || 0) - 1);
            break;
        }
        case 'lightningGun': {
            const damage = tier >= 2 ? 2 : 1;
            createProjectile(scene, origin.fireX, origin.fireY, Math.cos(baseAngle) * 900, Math.sin(baseAngle) * 900, 'overdrive', damage, { piercing: true });
            state.ammo.lightningGun = Math.max(0, (state.ammo.lightningGun || 0) - playerState.fireRate);
            break;
        }
        case 'stingerDrone': {
            const damage = tier >= 2 ? 2 : 1;
            createProjectile(scene, origin.fireX, origin.fireY, Math.cos(baseAngle) * 620, Math.sin(baseAngle) * 620, 'drone', damage);
            if (tier >= 3) {
                createProjectile(scene, origin.fireX, origin.fireY, Math.cos(baseAngle - 0.22) * 620, Math.sin(baseAngle - 0.22) * 620, 'drone', damage);
                createProjectile(scene, origin.fireX, origin.fireY, Math.cos(baseAngle + 0.22) * 620, Math.sin(baseAngle + 0.22) * 620, 'drone', damage);
            }
            break;
        }
        ...
        default: {
            ...
            for (let i = 0; i < burstCount; i++) {
                ...
                scene.time.delayedCall(delay, () => {
                    createProjectile(scene, origin.fireX, origin.fireY, Math.cos(baseAngle) * 760, Math.sin(baseAngle) * 760, 'normal', damage);
                });
            }
            break;
        }
