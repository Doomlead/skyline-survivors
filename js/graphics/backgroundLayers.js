// Background layer builders to keep createBackground modular

function createSkyAndCityLayers(scene, { random, withWrappedX, extendedWidth }) {
    // ========================================
    ...
    for (let y = 0; y < CONFIG.worldHeight; y += bandHeight) {
        ...
        skyGradient.fillRect(-200, y, extendedWidth, bandHeight + 1);
    }
    ...
    for (let i = 0; i < 5; i++) {
        ...
        atmosphere.fillRect(-200, by, extendedWidth, 30 + random() * 40);
    }
    ...
    for (let copyOffset of [-CONFIG.worldWidth, 0, CONFIG.worldWidth]) {
        ...
        const copyRng = createBackgroundRNG(CONFIG.backgroundSeed || 1337);
    ...
    // Rubble piles and broken walls
    for (let i = 0; i < 18; i++) {
        const rx = (i * (CONFIG.worldWidth / 18)) + random() * 60;
        const ry = CONFIG.worldHeight - 85 + random() * 20;
        nearCity.fillStyle(0x1e1e28, 1);
        nearCity.fillRect(rx - 25, ry - 15, 50 + random() * 30, 10 + random() * 8);
    }

    // Nearby destroyed buildings
    for (let i = 0; i < 14; i++) {
        const bx = (i * (CONFIG.worldWidth / 14)) + random() * 50;
        ...
        for (let s = 0; s <= segs; s++) {
            nearCity.lineTo(
                left + (bWidth / segs) * s,
                top + random() * bHeight * 0.3
            );
        }

        nearCity.lineTo(left + bWidth, top + bHeight * 0.05);
            ...
            for (let wx = 8; wx < bWidth - 10; wx += 15) {
                if (random() > 0.3) {
                    ...
                    if (state > 0.85) {
                        ...
                        nearCity.fillStyle(0x0c0c16, 0.9);
                    }
        ...
        if (random() > 0.35) {
            ...
            nearCity.fillStyle(0xff2200, 0.35);
            nearCity.fillCircle(fx, fy, 22);
            nearCity.fillStyle(0xff5500, 0.55);
            nearCity.fillCircle(fx, fy, 14);
            nearCity.fillStyle(0xff8800, 0.75);
            nearCity.fillCircle(fx, fy - 3, 8);
            nearCity.fillStyle(0xffcc00, 0.9);
            nearCity.fillCircle(fx, fy - 5, 4);
            nearCity.fillStyle(0xffffaa, 0.8);
            nearCity.fillCircle(fx, fy - 7, 2);

            nearCity.fillStyle(0x1a1a1a, 0.25);
            nearCity.fillCircle(fx - 6, fy - 35, 20);
            nearCity.fillCircle(fx + 4, fy - 55, 24);
            nearCity.fillCircle(fx - 2, fy - 78, 27);
        }
    }
...
function createForegroundTerrain(scene, { random, withWrappedX, groundY }) {
    // ========================================
    ...
    for (let i = 0; i < 18; i++) {
        ...
        for (let h = 0; h < 5; h++) {
            ...
            terrain.fillRect(hx, hy, 8 + random() * 12, 4 + random() * 6);
        }
